export async function HomeView(client, UserID) {
  const HomeViewResponse = { code: null, message: null };
  try {
    ...
    HomeViewResponse.code = Response.code;
    HomeViewResponse.message = Response.message;
  } catch (error) {
    if (error.errorCode === "HTTP_REQUEST_FAILED") {
      if (error.response.code === 10 && error.response.message === "请先登录") {
        HomeViewResponse.code = error.response.code;
        HomeViewResponse.message = error.response.message;
      }
